...
for dict in accounts:
		api1 = ShoonyaApiPy()
		user1 = dict["user"]
		ans[user1] = api1.login(userid=dict["user"], password=dict["pwd"], twoFA=dict["twoFA"], vendor_code=dict["vc"], api_secret=dict["app_key"], imei=dict["imei"])
...
while True:
	...
	# pdb.set_trace()
		...
		def place_order(account):
			api1 = ShoonyaApiPy()
			ret = api1.login(userid=account["user"], password=account["pwd"], twoFA=account["twoFA"], vendor_code=account["vc"], api_secret=account["app_key"], imei=account["imei"])
			# ans[user1]
