...
from user.models import DnFyersUserCredsMaster
...
class BrokerStore(PostLoginAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        brokerCredsObj = DnFyersUserCredsMaster.objects.filter(user=user).values(
            'id', 'user', 'fy_id', 'app_id', 'quantity', 'secret_key', 'totp_key', 'pin', 'status')

        def get_table_keys():
            ins = DnFyersUserCredsMaster()
    ...
    def post(self, request):
        ...
        if not fy_id or not totp_key:
            raise Exception(12006)

        obj = DnFyersUserCredsMaster(fy_id=fy_id,
                                     user=user,
                                     app_id=app_id,
                                     totp_key=totp_key,
                                     pin=pin,
    ...
    def put(self, request):
        ...
        if updateType == "many":
            ...
            DnFyersUserCredsMaster.objects.all().update(status=new_status)
            ...
            credObj = DnFyersUserCredsMaster.objects.filter(id=id)
